...
//MainActivity：初始化連絡人清單，並透過onActivityResult()接收新的聯絡人資料
class MainActivity : AppCompatActivity() {
    private lateinit var adapter: MyAdapter   //初始化連絡人清單:MyAdapter
    private val contacts = ArrayList<Contact>() //定義連絡人資料為ArrayList<類別>
    //接收回傳資料
    override fun onActivityResult(requestCode: Int, resultCode: Int, data:
    Intent?) {
        super.onActivityResult(requestCode, resultCode, data)
        data?.extras?.let {
            if (requestCode == 1 && resultCode == Activity.RESULT_OK) { //requestCode請求代碼，RESULT結果
